...
# mlp.add(Dense(units=n_hidden1,activation='tanh',input_shape=(n_input,),bias_initializer='zeros'))
# mlp.add(Dense(units=n_hidden2,activation='tanh',bias_initializer='zeros'))
# mlp.add(Dense(units=n_hidden3,activation='tanh',bias_initializer='zeros'))
# mlp.add(Dense(units=n_hidden4,activation='tanh',bias_initializer='zeros'))
# mlp.add(Dense(units=n_output,activation='tanh',bias_initializer='zeros'))
# 신경망 학습#
#if n_epochs ==10 :

#    mlp.compile(loss='mean_squared_error',optimizer=Adam(learning_rate=0.001),metrics=['accuracy'])
#    hist=mlp.fit(x_train,y_train,batch_size=128,epochs=n_epochs,validation_data=(x_test,y_test),verbosee=2)
# 신경망 학습#
#if n_epochs ==10 :

mlp.compile(loss='categorical_crossentropy',optimizer=SGD(),metrics=['accuracy'])
hist=mlp.fit(x_train,y_train,batch_size=128,epochs=10,validation_data=(x_test,y_test),verbose=2)
print("SGD")
